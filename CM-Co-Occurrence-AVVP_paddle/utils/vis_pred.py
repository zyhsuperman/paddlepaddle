...
results = {'Vision ': [2, 2, 2, 2, 2, 7, 7, 7, 9, 9], 'Audio ': [24, 24, 12,
    12, 7, 7, 7, 7, 9, 9], 'Vision+Audio ': [24, 19, 19, 12, 7, 7, 7, 7, 15,
    15]}
...
def visualization_temproal(results, path):
    ...
    category_names : list of str
        The category labels.
    """
    category_names = ['Speech', 'Car', 'Cheering', 'Dog', 'Cat',
    ...
    category_colors = plt.get_cmap('hsv')(np.linspace(0, 1, len(
        category_names)))
    ...
    for legend_idx in range(25):
        ax.barh(labels, 0, left=0, height=0.5, label=category_names[
            legend_idx], color=category_colors[legend_idx])
    for idx in range(10):
        colname = mapping_label(data[:, idx], category_names)
        color = mapping_label(data[:, idx], category_colors)
        ax.barh(labels, 13.5, left=idx * 13.5, height=0.5, color=np.array(
            color))
    ax.legend(ncol=10, bbox_to_anchor=(0, 1.15), loc='upper left', fontsize
        ='small')
    plt.savefig(path)
