...
def segment_level(SO_a, SO_v, SO_av, GT_a, GT_v, GT_av):
    ...
    if len(F_av) == 0:
        ...
        f_av = sum(F_av) / len(F_av)
    return f_a, f_v, f, f_av, (FN_a.sum(), FP_a.sum()), (FN_v.sum(), FP_v.sum()
        ), (FN_av.sum(), FP_av.sum()), (TP_a_only.sum(), gt_a_only.sum(),
        TP_v_only.sum(), gt_v_only.sum()), (TP_a, FN_a, FP_a, TP_v, FN_v, FP_v)
