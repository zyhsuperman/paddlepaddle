...
def _expand_global_features(B, T, g, bct=True):
    ...
    if g is None:
        return None
    g = paddle.unsqueeze(g, axis=-1) if len(g.shape) == 2 else g
    if bct:
        g_bct = paddle.expand(g, shape=[B, -1, T])
        return g_bct
    else:
        g_btc = paddle.transpose(paddle.expand(g, shape=[B, -1, T]), perm=[0, 2, 1])
        return g_btc
...
class WaveNet(paddle.nn.Layer):
    ...
    def __init__(self, out_channels=256, layers=20, stacks=2,
        ...
        self.last_conv_layers = paddle.nn.LayerList(sublayers=[paddle.nn.
            ReLU(), Conv1d1x1(skip_out_channels, skip_out_channels,
            weight_normalization=weight_normalization), paddle.nn.ReLU(),
            Conv1d1x1(skip_out_channels, out_channels, weight_normalization
        ...
        if upsample_conditional_features:
            self.upsample_conv = paddle.nn.LayerList()
            for s in upsample_scales:
                ...
                self.upsample_conv.append(paddle.nn.ReLU())
        else:
    ...
    def forward(self, x, c=None, g=None, softmax=False):
        ...
        if g is not None:
            if self.embed_speakers is not None:
                g = self.embed_speakers(g.reshape([B, -1]))

                g = paddle.transpose(g, perm=[0, 2, 1])
                assert g.dim() == 3
        g_bct = _expand_global_features(B, T, g, bct=True)
        if c is not None and self.upsample_conv is not None:
            c = c.unsqueeze(axis=1)
        ...
        for f in self.last_conv_layers:
            x = f(x)
        x = paddle.nn.functional.softmax(x=x, axis=1) if softmax else x
        return x
    ...
    def incremental_forward(self, initial_input=None, c=None, g=None, T=100,
        ...
        if test_inputs is not None:
            if self.scalar_input:
                if test_inputs.shape[1] == 1:
                    test_inputs = paddle.transpose(test_inputs, perm=[0, 2, 1])
            elif test_inputs.shape[1] == self.out_channels:
                test_inputs = paddle.transpose(test_inputs, perm=[0, 2, 1])
        ...
        if c is not None and self.upsample_conv is not None:
            ...
            c = c.squeeze(axis=1)
            assert c.shape[-1] == T
        if c is not None and c.shape[-1] == T:
            c = paddle.transpose(c, perm=[0, 2, 1])

        outputs = []
        if initial_input is None:
            if self.scalar_input:
                initial_input = paddle.zeros(shape=[B, 1, 1])
            else:
                initial_input = paddle.zeros(shape=[B, 1, self.out_channels])
                initial_input[:, :, (127)] = 1
            if 'gpu' in str(next(self.parameters()).place):
                initial_input = initial_input
        else:
            if initial_input.shape[1] == self.out_channels:
                initial_input = paddle.transpose(initial_input, perm=[0, 2, 1])

        current_input = initial_input
        for t in tqdm(range(T)):
            if test_inputs is not None and t < test_inputs.shape[1]:
                current_input = test_inputs[:, (t), :].unsqueeze(axis=1)
            elif t > 0:
                current_input = outputs[-1]
            ct = None if c is None else c[:, (t), :].unsqueeze(axis=1)
            gt = None if g is None else g_btc[:, (t), :].unsqueeze(axis=1)
            x = current_input
            ...
            for f in self.last_conv_layers:
                try:
                    ...
                    x = f(x)
            if self.scalar_input:
                x = sample_from_discretized_mix_logistic(x.reshape([B, -1, 1]),
                    log_scale_min=log_scale_min)
            else:
                x = paddle.nn.functional.softmax(x=x.reshape([B, -1]), axis=1
                    ) if softmax else x.reshape([B, -1])
                if quantize:
                    sample = np.random.choice(np.arange(self.out_channels),
                        p=x.reshape([-1]).data.cpu().numpy())
                    x.zero_()
                    x[:, (sample)] = 1.0
            outputs += [x.data]
        outputs = paddle.stack(x=outputs)

        outputs = paddle.transpose(outputs, perm=[1, 2, 0])
        self.clear_buffer()
        return outputs
    ...
    def make_generation_fast_(self):
        ...
        def remove_weight_norm(m):
            try:
                paddle.nn.utils.remove_weight_norm(layer=m)
            except ValueError:
                return
