...
class Utterances(paddle.io.Dataset):
    ...
    def __init__(self, root_dir, len_crop):
        ...
        for i in range(0, len(meta), self.step):
            p = Process(target=self.load_data, args=(meta[i:i + self.step],
                dataset, i))
...
def get_loader(root_dir, batch_size=16, len_crop=128, num_workers=0):
    ...
    worker_init_fn = lambda x: np.random.seed(paddle.get_rng_state()[0].
        current_seed() % 2 ** 32)
