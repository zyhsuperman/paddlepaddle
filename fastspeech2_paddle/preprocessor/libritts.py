    ...
    for speaker in tqdm(os.listdir(in_dir)):
        for chapter in os.listdir(os.path.join(in_dir, speaker)):
            for file_name in os.listdir(os.path.join(in_dir, speaker, chapter)
                ...
                wav_path = os.path.join(in_dir, speaker, chapter, '{}.wav'.
                    format(base_name))
                ...
                wavfile.write(os.path.join(out_dir, speaker, '{}.wav'.
                    format(base_name)), sampling_rate, wav.astype(np.int16))
                with open(os.path.join(out_dir, speaker, '{}.lab'.format(
                    base_name)), 'w') as f1:
                    f1.write(text)
