...
class Dataset(paddle.io.Dataset):
    ...
    def __init__(self, filename, preprocess_config, train_config, sort=
        ...
        self.basename, self.speaker, self.text, self.raw_text = (self.
            process_meta(filename))
    ...
    def __getitem__(self, idx):
        ...
        sample = {'id': basename, 'speaker': speaker_id, 'text': phone,
            'raw_text': raw_text, 'mel': mel, 'pitch': pitch, 'energy':
            energy, 'duration': duration}
        return sample

    def process_meta(self, filename):
        with open(os.path.join(self.preprocessed_path, filename), 'r',
            encoding='utf-8') as f:
            name = []
            speaker = []
            text = []
    ...
    def reprocess(self, data, idxs):
        ...
        return ids, raw_texts, speakers, texts, text_lens, max(text_lens
            ), mels, mel_lens, max(mel_lens), pitches, energies, durations
...
class TextDataset(paddle.io.Dataset):
    ...
    def __init__(self, filepath, preprocess_config):
        self.cleaners = preprocess_config['preprocessing']['text'][
            'text_cleaners']
        self.basename, self.speaker, self.text, self.raw_text = (self.
            process_meta(filepath))
        with open(os.path.join(preprocess_config['path'][
            'preprocessed_path'], 'speakers.json')) as f:
            self.speaker_map = json.load(f)
...
if __name__ == '__main__':
    ...
    train_loader = paddle.io.DataLoader(
        ...
        shuffle=True,
        collate_fn=train_dataset.collate_fn,  # Ensure this function is compatible with PaddlePaddle
    )
    ...
    val_loader = paddle.io.DataLoader(
        ...
        shuffle=False,
        collate_fn=val_dataset.collate_fn,  # Ensure this function is compatible with PaddlePaddle
    )
    n_batch = 0
