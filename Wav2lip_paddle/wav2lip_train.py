...
parser = argparse.ArgumentParser(description=
    'Code to train the Wav2Lip model without the visual quality discriminator')
parser.add_argument('--data_root', help=
    'Root folder of the preprocessed LRS2 dataset', required=True, type=str)
parser.add_argument('--checkpoint_dir', help=
    'Save checkpoints to this directory', required=True, type=str)
parser.add_argument('--syncnet_checkpoint_path', help=
    'Load the pre-trained Expert discriminator', required=True, type=str)
parser.add_argument('--checkpoint_path', help='Resume from this checkpoint',
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            mel = paddle.to_tensor(data=mel.T, dtype='float32').unsqueeze(axis
                =0)
            indiv_mels = paddle.to_tensor(data=indiv_mels, dtype='float32'
                ).unsqueeze(axis=1)
            y = paddle.to_tensor(data=y, dtype='float32')
            return x, indiv_mels, mel, y


def save_sample_images(x, g, gt, global_step, checkpoint_dir):
    x = (x.detach().cpu().numpy().transpose(0, 2, 3, 4, 1) * 255.0).astype(np
        .uint8)
    g = (g.detach().cpu().numpy().transpose(0, 2, 3, 4, 1) * 255.0).astype(np
        .uint8)
    gt = (gt.detach().cpu().numpy().transpose(0, 2, 3, 4, 1) * 255.0).astype(np
        .uint8)
    refs, inps = x[..., 3:], x[..., :3]
...
def train(model, train_data_loader, test_data_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        for step, (x, indiv_mels, mel, gt) in prog_bar:
            ...
            loss = hparams.syncnet_wt * sync_loss + (1 - hparams.syncnet_wt
                ) * l1loss
            loss.backward()
            optimizer.step()
            ...
            if global_step == 1 or global_step % hparams.eval_interval == 0:
                with paddle.no_grad():
                    average_sync_loss = eval_model(test_data_loader,
                        global_step, model, checkpoint_dir)
...
def eval_model(test_data_loader, global_step, model, checkpoint_dir):
    ...
    step = 0
    while 1:
        for x, indiv_mels, mel, gt in tqdm(test_data_loader, total=eval_steps):
            step += 1
            model.eval()
            # x = x.to(device)
            # gt = gt.to(device)
...
def save_checkpoint(model, optimizer, step, checkpoint_dir, epoch):
    checkpoint_path = join(checkpoint_dir, 'checkpoint_step{:09d}.pth'.
        format(global_step))
...
#         batch_size=hparams.batch_size, shuffle=True, num_workers=hparams.
#         num_workers)
# >>>>>>    test_data_loader = torch.utils.data.DataLoader(test_dataset, batch_size
#         =hparams.batch_size, num_workers=4)
    ...
    optimizer = paddle.optimizer.Adam(parameters=[p for p in model.
        ...
        initial_learning_rate, weight_decay=0.0)
    ...
    train(model, train_data_loader, test_data_loader, optimizer,
        ...
        checkpoint_interval, nepochs=hparams.nepochs)
