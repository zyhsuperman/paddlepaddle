...
parser.add_argument('--output_landmark_root', type=str, default=
    './lrs2_landmarks')
...
def _normalized_to_pixel_coordinates(normalized_x: float, normalized_y:
    float, image_width: int, image_height: int) ->Union[None, Tuple[int, int]]:
    """Converts normalized value pair to pixel coordinates."""

    def is_valid_normalized_value(value: float) ->bool:
        return (value > 0 or math.isclose(0, value)) and (value < 1 or math
            .isclose(1, value))
...
FACEMESH_RIGHT_IRIS = frozenset([(469, 470), (470, 471), (471, 472), (472, 
    469)])
...
FACEMESH_NOSE = frozenset([(168, 6), (6, 197), (197, 195), (195, 5), (5, 4),
    ...
    344), (344, 278)])
FACEMESH_FULL = frozenset().union(*[FACEMESH_LIPS, FACEMESH_LEFT_EYE,
...
pose_landmark_idx = summarize_landmarks(FACEMESH_NOSE.union(*[
...
def draw_landmarks(image: np.ndarray, landmark_list: landmark_pb2.
    NormalizedLandmarkList, connections: Optional[List[Tuple[int, int]]]=
    None, landmark_drawing_spec: Union[DrawingSpec, Mapping[int,
    DrawingSpec]]=DrawingSpec(color=RED_COLOR), connection_drawing_spec:
  ...
  """
    ...
    for idx, landmark in enumerate(landmark_list.landmark):
        if landmark.HasField('visibility'
            ) and landmark.visibility < _VISIBILITY_THRESHOLD or landmark.HasField(
            'presence') and landmark.presence < _PRESENCE_THRESHOLD:
            continue
        landmark_px = _normalized_to_pixel_coordinates(landmark.x, landmark
            .y, image_cols, image_rows)
        if landmark_px:
    ...
    if connections:
        ...
        for connection in connections:
            ...
            if not (0 <= start_idx < num_landmarks and 0 <= end_idx <
                num_landmarks):
                raise ValueError(
                    f'Landmark index is out of range. Invalid connection from landmark #{start_idx} to landmark #{end_idx}.'
                    )
            if (start_idx in idx_to_coordinates and end_idx in
                idx_to_coordinates):
...
def process_video_file(mp4_path):
    with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1,
        ...
        for frame_idx, full_frame in enumerate(frames):
            ...
            results = face_mesh.process(cv2.cvtColor(full_frame, cv2.
                COLOR_BGR2RGB))
            ...
            face_frame = cv2.resize(full_frame[int(y_min * h):int(y_max * h
                ), int(x_min * w):int(x_max * w)], (128, 128))
            pose_landmarks = [(idx, (x - x_min) / (x_max - x_min), (y -
                y_min) / (y_max - y_min)) for idx, x, y in pose_landmarks]
            ...
            out_dir = os.path.join(output_landmark_root, '/'.join(mp4_path[
                :-4].split('/')[-2:]))
            os.makedirs(out_dir, exist_ok=True)
            np.save(os.path.join(out_dir, str(frame_idx)), result_dict)
            h_new = (y_max - y_min) * h
            w_new = (x_max - x_min) * w
            annotated_image = np.zeros((int(h_new * 128 / min(h_new, w_new)
                ), int(w_new * 128 / min(h_new, w_new)), 3))
            draw_landmarks(image=annotated_image, landmark_list=
                face_landmarks, connections=FACEMESH_FULL,
                connection_drawing_spec=drawing_spec)
            annotated_image = cv2.resize(annotated_image, (128, 128))
            out_dir = os.path.join(output_sketch_root, '/'.join(mp4_path[:-
                4].split('/')[-2:]))
            os.makedirs(out_dir, exist_ok=True)
            cv2.imwrite(os.path.join(out_dir, str(frame_idx) + '.png'),
                annotated_image)
            out_dir = os.path.join(output_face_root, '/'.join(mp4_path[:-4]
                .split('/')[-2:]))
            os.makedirs(out_dir, exist_ok=True)
            cv2.imwrite(os.path.join(out_dir, str(frame_idx) + '.png'),
                face_frame)
...
def main():
    ...
    mp4_list = glob.glob(input_mp4_root + '/*/*.mp4')
    print('total videos :', len(mp4_list))
    process_num = args.process_num
    print('process_num: ', process_num)
    p_frames = ThreadPoolExecutor(process_num)
    futures_frames = [p_frames.submit(mp_handler, mp4_path) for mp4_path in
        mp4_list]
    _ = [r.result() for r in tqdm(as_completed(futures_frames), total=len(
        futures_frames))]
    print('complete task!')
