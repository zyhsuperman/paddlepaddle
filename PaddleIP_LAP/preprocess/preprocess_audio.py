import sys
sys.path.append('..')
from PaddleIP_LAP.models import audio
from os import path
...
parser.add_argument('--data_root', type=str, help=
    'Root folder of the LRS2 dataset', required=True)
...
def process_audio_file(vfile, args):
    ...
    command = template.format(vfile.replace(' ', '\\ '), wavpath.replace(
        ' ', '\\ '))
...
def main(args):
    ...
    _ = [r.result() for r in tqdm(as_completed(futures_audio), total=len(
        futures_audio))]
