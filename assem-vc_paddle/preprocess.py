...
def _process_speaker(filepath, sampling_rate, frame_length, hop_length,
    f0_min, f0_max, harm_thresh):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output_filename', type=str, default=
        'f0s.txt', help='name of the output file')
    args = parser.parse_args()
    hp = OmegaConf.load(args.config)
    with open(os.path.join(hp.data.train_dir, hp.data.train_meta), 'r',
        encoding='utf-8') as g:
        data = g.readlines()
    ...
    means = [(0.0) for i in range(n)]
    frame_count = [(0) for i in range(n)]
    for i, fpath in enumerate(tqdm.tqdm(wavdir)):
        spk_idx = speaker_to_idx[speaker[i]]
        square, sum, length = _process_speaker(os.path.join(hp.data.
            train_dir, fpath), hp.audio.sampling_rate, hp.audio.
            filter_length, hp.audio.hop_length, hp.audio.f0_min, hp.audio.
            f0_max, hp.audio.harm_thresh)
