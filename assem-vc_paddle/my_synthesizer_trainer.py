...
class MySynthesizerTrainer:
    ...
    def training_step(self, batch):
        (text, mel_source, speakers, f0_padded, input_lengths,
            output_lengths, max_input_len, _) = batch
        with paddle.no_grad():
    ...
    def validation_step(self, batch):
        ...
        ling_s, alignment = self.model(text, mel_source, input_lengths,
            output_lengths, max_input_len)
        z_s = self.model.speaker(mel_source, output_lengths)
        rand_batch_idx = np.random.randint(z_s.shape[0], size=z_s.shape[0])
    ...
    def configure_optimizers(self):
        optimizer = paddle.optimizer.Adam(parameters=list(self.model.
            decoder.parameters()) + list(self.model.f0_encoder.parameters()
            ) + list(self.model.speaker.parameters()), learning_rate=self.
            hp.train.adam.lr, weight_decay=self.hp.train.adam.weight_decay)
        return optimizer

    def train_dataloader(self):
        trainset = TextMelDataset(self.hp, self.hp.data.train_dir, self.hp.
            data.train_meta, train=True, norm=False, use_f0s=True)
...
if __name__ == '__main__':
    ...
    trainer.train_and_val("checkpoints/assem-vc_pretrained.pdparams")
