...
def plot_alignments(alignment1, alignment2, transpose=True):
    ...
    plt.subplot(211)
    plt.imshow(alignment1, aspect='auto', origin='lower', interpolation=
        'none', norm=Normalize(vmin=0.0, vmax=1.0))
    plt.ylabel('Encoder timestep')
    plt.subplot(212)
    plt.imshow(alignment2, aspect='auto', origin='lower', interpolation=
        'none', norm=Normalize(vmin=0.0, vmax=1.0))
...
def plot_spectrograms(mel_pred, mel_postnet, mel_target):
    ...
    plt.subplot(311)
    plt.imshow(mel_pred, aspect='auto', origin='lower', interpolation=
        'none', norm=Normalize(vmin=-5.0, vmax=1.0))
    plt.ylabel('before postnet')
    plt.subplot(312)
    plt.imshow(mel_postnet, aspect='auto', origin='lower', interpolation=
        'none', norm=Normalize(vmin=-5.0, vmax=1.0))
    plt.ylabel('after postnet')
    plt.subplot(313)
    plt.imshow(mel_target, aspect='auto', origin='lower', interpolation=
        'none', norm=Normalize(vmin=-5.0, vmax=1.0))
...
def plot_conversions(mel_source, mel_s_s, mel_s_t):
    fig = plt.figure(figsize=(12, 9))
    plt.subplot(311)
    plt.imshow(mel_source, aspect='auto', origin='lower', interpolation=
        'none', norm=Normalize(vmin=-5.0, vmax=1.0))
