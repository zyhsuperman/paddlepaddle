...
class TextMelDataset(paddle.io.Dataset):
    ...
    def get_f0(self, audio, f0_mean=None, f0_var=None, sampling_rate=22050,
        frame_length=1024, hop_length=256, f0_min=80, f0_max=880,
        harm_thresh=0.25, mel_fmin=70.0):
        """f0, harmonic_rates, argmins, times = compute_yin(
            audio, sampling_rate, frame_length, hop_length, f0_min, f0_max,
            harm_thresh, mel_fmin)"""
        f0 = sptk.rapt(audio * 32768, sampling_rate, hop_length, min=f0_min,
            max=f0_max, otype=2)
    ...
    def get_mel_and_f0(self, audiopath, f0_mean=None, f0_var=None):
        ...
        wav = paddle.to_tensor(data=wav)
        wav = wav.unsqueeze(axis=0)
        if self.norm:
            wav = wav * (0.99 / (paddle.max(x=paddle.abs(x=wav)) + 1e-07))
        mel = mel_spectrogram(wav, self.hp.audio.filter_length, self.hp.
            audio.n_mel_channels, self.hp.audio.sampling_rate, self.hp.
            audio.hop_length, self.hp.audio.win_length, self.hp.audio.
            mel_fmin, self.hp.audio.mel_fmax, center=False)
        mel = mel.squeeze(axis=0)
        wav = wav.cpu().numpy()[0]
        if self.use_f0s:
            f0 = self.get_f0(wav, f0_mean, f0_var, self.hp.audio.
                sampling_rate, self.hp.audio.filter_length, self.hp.audio.
                hop_length, self.f0_min, self.f0_max, self.harm_thresh,
                self.mel_fmin)
            f0 = paddle.to_tensor(data=f0)[None]
            f0 = f0[:, :mel.shape[1]]
            return mel, f0
        return mel
    ...
    def load_f0s_lists(self, path, split='|'):
        ...
        assert f0s_spk_list == self.hp.data.speakers, 'speaker list mismatch in f0s_list: expected %s, but %s at %s' % (
...
def text_mel_collate(batch):
    ...
    if len(batch[0]) == 5:
        use_f0s = True
        f0_padded = paddle.empty(shape=[len(batch), 1, max_target_len],
            dtype='float32')
        f0_padded.zero_()
    for idx, key in enumerate(ids_sorted_decreasing):
        ...
        if use_f0s:
            f0 = batch[key][4]
            f0_padded[(idx), :, :f0.shape[1]] = f0
    if use_f0s:
        return (text_padded, mel_padded, speakers, f0_padded, input_lengths,
            output_lengths, max_input_len, new_paths)
