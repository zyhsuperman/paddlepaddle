...
#         normalized=False, onesided=True)
    spec = paddle.signal.stft(y, n_fft=n_fft, hop_length=hop_size, win_length=win_size, 
                          window=hann_window[str(y.place)], center=center, 
...
class MelDataset(paddle.io.Dataset):
    ...
    def __init__(self, training_files, segment_size, n_fft, num_mels,
        hop_size, win_size, sampling_rate, fmin, fmax, split=True, shuffle=
        True, n_cache_reuse=1, device=None, fmax_loss=None, fine_tuning=
        False, base_mels_path=None):
    ...
    def __getitem__(self, index):
        ...
        audio = paddle.to_tensor(data=audio, dtype='float32')
        audio = audio.unsqueeze(axis=0)
        if not self.fine_tuning:
            if self.split:
                if audio.shape[1] >= self.segment_size:
                    max_audio_start = audio.shape[1] - self.segment_size
                    audio_start = random.randint(0, max_audio_start)
                    audio = audio[:, audio_start:audio_start + self.
                        segment_size]
                else:
                    audio = paddle.nn.functional.pad(audio, [0, 0, 0, 0, 0, self.segment_size - audio.shape[1]], 'constant')
            ...
            mel = np.load(os.path.join(self.base_mels_path, os.path.
                splitext(os.path.split(filename)[-1])[0] + '.npy'))
            mel = paddle.to_tensor(data=mel)
            if len(mel.shape) < 3:
                mel = mel.unsqueeze(axis=0)
            if self.split:
                frames_per_seg = math.ceil(self.segment_size / self.hop_size)
                if audio.shape[1] >= self.segment_size:
                    mel_start = random.randint(0, mel.shape[2] -
                        frames_per_seg - 1)
                    mel = mel[:, :, mel_start:mel_start + frames_per_seg]
                    audio = audio[:, mel_start * self.hop_size:(mel_start +
                        frames_per_seg) * self.hop_size]
                else:
                    mel = paddle.nn.functional.pad(mel, [0, 0, 0, 0, 0, frames_per_seg - mel.shape[2]], 'constant')
                    audio = paddle.nn.functional.pad(audio, [0, 0, 0, 0, 0, self.segment_size - audio.shape[1]], 'constant')
        mel_loss = mel_spectrogram(audio, self.n_fft, self.num_mels, self.
            sampling_rate, self.hop_size, self.win_size, self.fmin, self.
            fmax_loss, center=False)
        return mel.squeeze(), audio.squeeze(axis=0
            ), filename, mel_loss.squeeze()
