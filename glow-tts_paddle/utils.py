...
def load_checkpoint(checkpoint_path, model, optimizer=None):
    ...
    checkpoint_dict = paddle.load(path=checkpoint_path)
    iteration = 1
    ...
    if optimizer is not None and 'optimizer' in checkpoint_dict.keys():
        optim_state_dict = checkpoint_dict['optimizer']
        for key in optim_state_dict:
            if key in optimizer.state_dict():
                optimizer.state_dict()[key].set_value(optim_state_dict[key])
...
def summarize(writer, global_step, scalars={}, histograms={}, images={}):
    ...
    for k, v in images.items():
        writer.add_image(k, v, global_step)
...
def plot_alignment_to_numpy(alignment, info=None):
    ...
    im = ax.imshow(alignment, aspect='auto', origin='lower', interpolation=
        'none')
...
def get_hparams(init=True):
    ...
    parser.add_argument('-c', '--config', type=str, default=
        './configs/base.json', help='JSON file for configuration')
