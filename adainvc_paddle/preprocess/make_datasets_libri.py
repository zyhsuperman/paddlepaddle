...
def get_speaker2path(root_dir, dset):
    ...
    for path in sorted(glob.glob(os.path.join(root_dir, f'{dset}/*/*/*.wav'))):
        ...
        speaker_id = re.match('(\\d+)_(\\d+)_(\\d+)_(\\d+)\\.wav', filename
            ).groups()[0]
...
if __name__ == '__main__':
    ...
    print(
        f'{len(train_paths)} training data, {len(dev_paths)} dev data, {len(test_paths)} test data'
        )
    ...
    with open(os.path.join(output_dir, 'test_files.txt'), 'w') as f:
        for path in sorted(test_paths):
            filename = path.strip().split('/')[-1]
            f.write(f'{filename}\n')
    for dset, paths in zip(['train', 'dev', 'test'], [train_paths,
