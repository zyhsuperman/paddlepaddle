...
def get_LLP_dataloader(args):
    ...
    train_dataset = LLP_dataset(label=args.label_train, audio_dir=args.audio_dir,
                                ...
                                transform=Compose([ToTensor()]),  # 确保转换为 PaddlePaddle 格式
...
def get_random_state():
    state = {
        'paddle_rng': paddle.get_cuda_rng_state(),
...
def train_noise_estimator(args, model, train_loader, optimizer, criterion,
    epoch, logger):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        Pa, Pv = sample['Pa'].astype(paddle.float32), sample['Pv'
        ...
        output, a_prob, v_prob, frame_prob, sims_after, mask_after = model(
            audio, video, video_st, with_ca=False)
        output = paddle.clip(x=output, min=1e-07, max=1 - 1e-07)
        a_prob = paddle.clip(x=a_prob, min=1e-07, max=1 - 1e-07)
        v_prob = paddle.clip(x=v_prob, min=1e-07, max=1 - 1e-07)
        ...
        if batch_idx % args.log_interval == 0:
            print(
                'Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss1: {:.3f}\tLoss2: {:.3f}\tLoss3: {:.3f}\tLoss4: {:.3f}'
                .format(epoch, batch_idx * len(audio), len(train_loader.
                dataset), 100.0 * batch_idx / len(train_loader), loss1.item
                (), loss2.item(), loss3.item(), loss4.item()))
        if logger:
            logger.add_scalar('loss', loss, global_step=(epoch - 1) * len(
                train_loader) + batch_idx + 1)
            logger.add_scalar('loss_audio', loss1, global_step=(epoch - 1) *
                len(train_loader) + batch_idx + 1)
            logger.add_scalar('loss_visual', loss2, global_step=(epoch - 1) *
                len(train_loader) + batch_idx + 1)
            logger.add_scalar('loss_global', loss3, global_step=(epoch - 1) *
                len(train_loader) + batch_idx + 1)
            logger.add_scalar('loss_nce', loss4, global_step=(epoch - 1) *
                len(train_loader) + batch_idx + 1)
...
def calculate_noise_ratio(args, model, train_loader):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        output, a_prob, v_prob, frame_prob = model(audio, video, video_st,
            with_ca=False)[:4]
        a_prob_list.append(paddle.mean(x=a_prob, axis=0).detach().cpu().numpy()
            )
        v_prob_list.append(paddle.mean(x=v_prob, axis=0).detach().cpu().numpy()
            )
        da = {'a': a_prob.cpu().detach(), 'v': v_prob.cpu().detach(),
            'label': target.cpu(), 'Pa': Pa.cpu().detach(), 'Pv': Pv.cpu().
            detach()}
        datas.append(da)
        if batch_idx % args.log_interval == 0:
            print('Estimate epoch: 1 [{}/{} ({:.0f}%)]'.format(batch_idx *
                len(audio), len(train_loader.dataset), 100.0 * batch_idx /
                len(train_loader)))
    ...
    labels = pd.read_csv('data/AVVP_train.csv', header=0, sep='\t')[
        'event_labels'].values
...
def train_label_denoising(args, model, train_loader, optimizer, criterion,
    epoch, logger):
    ...
    noise_ratios_a_init = paddle.to_tensor(data=noise_ratios['audio'])
    noise_ratios_v_init = paddle.to_tensor(data=noise_ratios['visual'])
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        with paddle.no_grad():
            ...
            a_prob = paddle.clip(a_prob, min=args.clamp, max=1 - args.clamp)
            v_prob = paddle.clip(v_prob, min=args.clamp, max=1 - args.clamp)

            tmp_loss_a = paddle.nn.functional.binary_cross_entropy(a_prob, Pa, reduction='none')
            tmp_loss_v = paddle.nn.functional.binary_cross_entropy(v_prob, Pv, reduction='none')
            _, sort_index_a = paddle.topk(tmp_loss_a, k=tmp_loss_a.shape[0], axis=0, largest=False)
            _, sort_index_v = paddle.topk(tmp_loss_v, k=tmp_loss_v.shape[0], axis=0, largest=False)

            pos_index_a = Pa > 0.5
            pos_index_v = Pv > 0.5

            for i in range(25):
                pos_num_a = paddle.sum(pos_index_a[:, i].astype(paddle.float64))
        ...
        optimizer.clear_grad()
        output, a_prob, v_prob, frame_prob, sims_after, mask_after = model(
            audio, video, video_st, with_ca=True)
        output = paddle.clip(x=output, min=args.clamp, max=1 - args.clamp)
        a_prob = paddle.clip(x=a_prob, min=args.clamp, max=1 - args.clamp)
        v_prob = paddle.clip(x=v_prob, min=args.clamp, max=1 - args.clamp)
        loss1 = criterion(a_prob, Pa)
        loss2 = criterion(v_prob, Pv)
        loss3 = criterion(output, target)
        loss4 = criterion2(sims_after, mask_after)
...
def eval(args, model, val_loader, set):
    ...
    with paddle.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            audio, video, video_st, target = sample['audio'].cuda(), \
                                             sample['video_s'].cuda(), \
                                             sample['video_st'].cuda(), \
                                             sample['label'].cuda()
            output, a_prob, v_prob, frame_prob = model(audio, video, video_st, with_ca=args.with_ca)[:4]
            oa = (a_prob.numpy() >= 0.5).astype(np.int_)
            ov = (v_prob.numpy() >= 0.5).astype(np.int_)

            Pa = frame_prob[0, :, 0, :].numpy()
            Pv = frame_prob[0, :, 1, :].numpy()
            # 过滤带预测弱标签的假阳性事件
            ...
            filename = df.iloc[batch_idx, 0]  # 正确获取单元格数据
            ...
            SO_av = SO_a * SO_v
            f_a, f_v, f, f_av = segment_level(SO_a, SO_v, SO_av, GT_a, GT_v,
                GT_av)
            F_seg_a.append(f_a)
            F_seg_v.append(f_v)
            F_seg.append(f)
    ...
    (audio_segment_level, visual_segment_level, av_segment_level, avg_type,
        avg_event, audio_event_level, visual_event_level, av_event_level,
        avg_type_event, avg_event_level) = (print_overall_metric(F_seg_a,
        F_seg_v, F_seg, F_seg_av, F_event_a, F_event_v, F_event, F_event_av))
    return (audio_segment_level, visual_segment_level, av_segment_level,
        avg_type, avg_event, audio_event_level, visual_event_level,
        av_event_level, avg_type_event, avg_event_level)


def main():
    parser = argparse.ArgumentParser(description=
        'PyTorch Implementation of Audio-Visual Video Parsing')
    parser.add_argument('--audio_dir', type=str, default=
        'data/feats/vggish/', help='audio dir')
    parser.add_argument('--video_dir', type=str, default=
        'data/feats/res152/', help='video dir')
    parser.add_argument('--st_dir', type=str, default=
    ...
    parser.add_argument('--label_val', type=str, default=
    ...
    parser.add_argument('--warm_up_epoch', type=float, default=0.9, help=
        'warm-up epochs')
    ...
    parser.add_argument('--clamp', type=float, default=1e-07)
    parser.add_argument('--nce_smooth', type=float, default=0.1)
    parser.add_argument('--temperature', type=float, default=0.2, help=
        'feature temperature number')
    parser.add_argument('--log_interval', type=int, default=700, help=
        'how many batches for logging training status')
    parser.add_argument('--log_file', type=str, help='log file path')
    parser.add_argument('--save_model', type=str, choices=['true', 'false'],
        help='whether to save model')
    parser.add_argument('--model_save_dir', type=str, default='ckpt/', help
        ='model save dir')
    parser.add_argument('--checkpoint', type=str, default='MMIL_Net', help=
        'save model name')
    ...
    if args.mode == 'train_noise_estimator':
        ...
        for epoch in range(1, args.epochs + 1):
            train_noise_estimator(args, model, train_loader, optimizer,
                criterion, epoch=epoch, logger=None)
        ...
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size,
                                shuffle=True, num_workers=5, use_shared_memory=True)
        resume = paddle.load(path=os.path.join(args.model_save_dir, args.
            checkpoint))
        ...
        best_epoch = 0
        for epoch in range(1, args.epochs + 1):
            train_label_denoising(args, model, train_loader, optimizer,
                criterion, epoch=epoch, logger=None)
            scheduler.step(epoch)
            print('Validation Performance of Epoch {}:'.format(epoch))
            (audio_seg, visual_seg, av_seg, avg_type_seg, avg_event_seg,
        ...
        test_dataset = LLP_dataset(label=dataset, audio_dir=args.audio_dir,
            video_dir=args.video_dir, st_dir=args.st_dir, transform=
            paddle.vision.transforms.Compose([ToTensor()]))
        test_loader = paddle.io.DataLoader(dataset=test_dataset, batch_size
            =1, shuffle=False, num_workers=1)
        resume = paddle.load(path=os.path.join(args.model_save_dir, args.
            checkpoint))
        for k, v in resume['model'].items():
            print(type(v))
        model.set_state_dict(state_dict=resume['model'])
