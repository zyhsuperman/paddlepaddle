...
def print_overall_metric(F_seg_a, F_seg_v, F_seg, F_seg_av, F_event_a,
    F_event_v, F_event, F_event_av):
    ...
    avg_type_seg = (100 * np.mean(np.array(F_seg_av)) + 100 * np.mean(np.
        array(F_seg_a)) + 100 * np.mean(np.array(F_seg_v))) / 3.0
    ...
    avg_type_eve = (100 * np.mean(np.array(F_event_av)) + 100 * np.mean(np.
        array(F_event_a)) + 100 * np.mean(np.array(F_event_v))) / 3.0
    avg_event_eve = 100 * np.mean(np.array(F_event))
    print(
        'Audio Event Detection Segment-level F1, Visual Event Detection Segment-level F1, Audio-Visual Event Detection Segment-level F1, Segment-level Type@Avg. F1, Segment-level Event@Avg. F1, Audio Event Detection Event-level F1, Visual Event Detection Event-level F1, Audio-Visual Event Detection Event-level F1, Event-level Type@Avg. F1, Event-level Event@Avg. F1'
        )
    print(
        '{:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}'
        .format(audio_segment_level, visual_segment_level, av_segment_level,
        avg_type_seg, avg_event_seg, audio_event_level, visual_event_level,
        av_event_level, avg_type_eve, avg_event_eve))
